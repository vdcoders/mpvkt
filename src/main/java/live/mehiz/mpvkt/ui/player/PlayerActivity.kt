...
class PlayerActivity : AppCompatActivity() {
    ...
    private fun getFileName(intent: Intent): String {
        ...
        if (uri.scheme == ContentResolver.SCHEME_CONTENT) {
            // 1. Try DocumentFile (usually reliable for SAF)
            DocumentFile.fromSingleUri(this, uri)?.name?.let {
                return it
            }

            // 2. Try OpenableColumns.DISPLAY_NAME
            contentResolver.query(uri, arrayOf(OpenableColumns.DISPLAY_NAME), null, null)?.use { cursor ->
                if (cursor.moveToFirst()) {
                    val index = cursor.getColumnIndex(OpenableColumns.DISPLAY_NAME)
                    if (index >= 0) {
                        val name = cursor.getString(index)
                        // If name is not null/blank and NOT just digits (which might be an ID), use it
            ...
            // 3. Try MediaStore TITLE (fallback for some providers)
            try {
                contentResolver.query(uri, arrayOf(MediaStore.MediaColumns.TITLE), null, null)?.use { cursor ->
                    if (cursor.moveToFirst()) {
                        val index = cursor.getColumnIndex(MediaStore.MediaColumns.TITLE)
                        if (index >= 0) {
    ...
    internal fun event(eventId: Int) {
        ...
        when (eventId) {
            MPVLib.mpvEventId.MPV_EVENT_FILE_LOADED -> {
                ...
                // FORCE the title to be the resolved filename, overriding any metadata
